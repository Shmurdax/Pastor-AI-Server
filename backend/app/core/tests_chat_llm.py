...
class VllmUrlResolutionTests(unittest.TestCase):
    ...
    def test_chat_view_uses_get_chat_llm_not_placeholder_key(self):
        ...
        self.assertNotIn("answer_char_count", source)
        self.assertIn('os.getenv("CHAT_MAX_HISTORY_CHARS", "20000")', source)
        self.assertIn('os.getenv("CHAT_MAX_TOKENS", "1024")', source)
        self.assertNotIn("answer_needs_expansion", source)
        self.assertNotIn("CONTINUE_STEER", source)
        self.assertNotIn("FINISH_STEER", source)
        self.assertNotIn("MAX_EXPANSION_PASSES", source)
        self.assertNotIn("_iter_continuation_tokens", source)
        self.assertNotIn("_trim_continuation_messages", source)
        self.assertNotIn("join_continuation", source)
        ...
        self.assertIn("compact_history_ai", source)
        self.assertNotIn("_claim_repair_plan", source)
        ...
        self.assertNotIn("keeping the first answer", source)
        self.assertIn("compact_history_ai(msg.ai_response or \"\")", source)
