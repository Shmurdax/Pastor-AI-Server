...
class ChatSystemPromptTests(unittest.TestCase):
    ...
    def test_prompt_requires_sermon_notes_and_long_paragraphs(self):
        ...
        self.assertIn("According to Pastor Don", prompt)
        self.assertIn("even if they never use", prompt.lower())
        self.assertIn("nearest related subject", prompt.lower())
        ...
        self.assertIn("Never write rewrite plans", prompt)
        self.assertIn("reshape the answer", prompt)
