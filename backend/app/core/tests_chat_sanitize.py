...
class ChatSanitizeTests(unittest.TestCase):
    ...
    def test_stream_delta_drops_han_for_english_generation(self):
        self.assertEqual(
            sanitize_stream_delta("self-esteem and重塑回答", language="en"),
            "self-esteem and",
        )
    ...
    def test_strips_mid_sentence_chinese_on_long_session_reply(self):
        leaked = (
    ...
    def test_keeps_english_lead_in_and_drops_splice_junk(self):
        leaked = (
            ...
            "宽容和怜悯\n"
            "首先，让我们考虑父亲的回应。他并没有严厉地评判他的儿子或惩罚他的错误。"
        )
        cleaned = sanitize_chat_answer(leaked, language="en")
        self.assertNotRegex(cleaned, r"[\u3400-\u9fff]")
        self.assertIn("pure grace", cleaned)
        self.assertIn("wronged us.", cleaned)
        self.assertNotIn("doesnfsp", cleaned)
        self.assertNotIn("宽容", cleaned)
    ...
    def test_history_keeps_user_chinese_question(self):
        question = "什么是饶恕？"
        self.assertEqual(
            sanitize_history_text(question, language="en", is_user=True),
            question,
        )
    ...
    def test_mostly_chinese_uses_fallback(self):
        out = sanitize_chat_answer("由于提供的参考文献中没有相关内容。")
        self.assertEqual(out, ENGLISH_LANGUAGE_FALLBACK)

    def test_recover_prefers_clean_retry(self):
        first = "Grace is a gift. 宽容和怜悯首先让我们考虑父亲的回应。"
        retry = (
            "God's forgiveness is not conditional on our performance. "
            "It is an act of pure grace."
        )
        answer, leaked = recover_english_generation(first, retry)
        self.assertFalse(leaked)
        self.assertEqual(answer, retry)
        self.assertNotIn("宽容", answer)

    def test_recover_keeps_english_lead_in_when_retry_still_leaks(self):
        first = (
            "God's forgiveness is not conditional on our performance or worthiness. "
            "It is an act of pure grace, extended freely to all who seek it. "
            "Just as the father forgave his son, we are called to forgive those who "
