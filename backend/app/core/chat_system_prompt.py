...
LENGTH_STEER = (
    ...
    "not a Scripture dump. Every Pastor Don or Susan quotation must be copied "
    "word-for-word from ALLOWED SERMON QUOTES. Every Scripture quotation must be "
    "copied word-for-word from ALLOWED NKJV. Do not invent kitchen speeches, "
    "new verses, or lines that are not on those lists. If the lists cannot "
    "answer, say so and only use the retrieved lines. Do not stop after one "
    "sentence. When the teaching is complete, end with a short pastoral close "
    "that restates the retrieved lines. Do not start that close with "
    ...
    "the retrieved quotes in this formatted ~2000-character teaching.\n\n"
    "User question:\n"
)
...
def build_chat_system_prompt(*, biblical_names: list[str] | None = None) -> str:
    ...
    return (
        ...
        "notes). Quote strongly related notes first. If the retrieved notes do not address the question, "
        "say that plainly. Do not invent a parallel, kitchen script, or extra doctrine to fill the gap. "
        "Never say notes were not found or missing when excerpts are present. Prefer a fresh quotation over "
        ...
        "mention. If the retrieved sermons do not mention it, say that plainly and only quote retrieved NKJV "
        "if an ALLOWED NKJV line is present.\n"
        "REQUIRED QUOTES: Every in-depth teaching answer (Christianity, theology, Bible, or social issues) "
        "must include direct, word-for-word quotations from Pastor Don Nordin and/or Susan Nordin copied from "
        "ALLOWED SERMON QUOTES (which come from retrieved sermon/video notes). Put their wording in quotation "
        "marks and attribute each quote clearly "
        ...
        "when ALLOWED SERMON QUOTES is not empty. "
        ...
        "Never invent, polish, or reconstruct quotes. Only quote wording that actually appears in "
        "ALLOWED SERMON QUOTES or ALLOWED NKJV. Every Scripture quotation must be copied from ALLOWED NKJV "
        "(the ingested NKJV document)—never from memory or another translation. "
        "If ALLOWED SERMON QUOTES is empty, do not invent a Pastor Don or Susan quotation.\n"
        "Never reply with a one-line brush-off such as \"No relevant sermon notes found.\" Only when "
        "REFERENCE NOTES are empty and ALLOWED NKJV is empty should you say you do not have retrieved "
        "notes for this question—do not invent a teaching to fill space.\n"
        "</source_material>\n\n"
        ...
        "Cover the notes, quote Pastor Don and/or Susan from ALLOWED SERMON QUOTES, quote only ALLOWED NKJV, "
        "and apply those lines—then stop. "
        "End with a short pastoral close that restates the retrieved lines. Do not start that close with "
        ...
        "- VERSION: Only quote Scripture copied from ALLOWED NKJV (the ingested NKJV document). "
