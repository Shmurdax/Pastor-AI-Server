...
grep -q '^CHAT_MAX_TOKENS=2048$' "$TMP" || fail "max tokens should be upserted so replies are not cut mid-sentence"
...
grep -q '^RETRIEVAL_CANDIDATE_MULTIPLIER=8$' "$TMP" || fail "candidate multiplier should inspect more hits"
grep -q '^RETRIEVAL_SOURCE_MIN=3$' "$TMP" || fail "source min should be upserted"
grep -q '^RETRIEVAL_SOURCE_MAX=5$' "$TMP" || fail "source max should be upserted"
