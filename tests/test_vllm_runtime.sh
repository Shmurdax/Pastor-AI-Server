...
grep -q '^RETRIEVAL_THRESHOLD=0.45$' "$TMP" || fail "retrieval threshold should be upserted for diverse RAG"
grep -q '^RETRIEVAL_MAX_PER_SOURCE=2$' "$TMP" || fail "max per source should be upserted"
